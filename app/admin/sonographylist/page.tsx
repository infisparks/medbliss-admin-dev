...
import { Eye, MapPin, Calendar, Clock, StickyNote } from 'lucide-react'
...
export default function SonographyBookingsListPage() {
  const [bookings, setBookings] = useState<SonographyBookingRow[]>([])
  const [filteredBookings, setFilteredBookings] = useState<SonographyBookingRow[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedBooking, setSelectedBooking] = useState<SonographyBookingRow | null>(null)
  ...
  const fetchBookings = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`*, profiles(full_name, phone), sonography_services(service_name)`)
  ...
  const handleSearch = (search: string) => {
    ...
    setFilteredBookings(bookings.filter(b =>
      (b.profiles?.full_name?.toLowerCase().includes(lower) ||
        ...
        b.sonography_services?.service_name?.toLowerCase().includes(lower))
    ))
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">Sonography Bookings</h1>
                  ...
                  <TableRow key={booking.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center">
                        ...
                        <span className="text-sm">{format(parseISO(booking.booking_date), 'MMM dd, yyyy')}</span>
                      </div>
                    ...
                    <TableCell>{booking.sonography_services?.service_name || 'N/A'}</TableCell>
                    <TableCell>
