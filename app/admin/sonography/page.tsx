...
import { Plus, Edit, Trash2, Camera, Clock, DollarSign } from 'lucide-react'
import { supabase } from '@/lib/supabase'
import { format } from 'date-fns'

interface SonographyService {
  id: string
  service_name: string
  description: string
  base_price: number
...
export default function SonographyPage() {
  const [services, setServices] = useState<SonographyService[]>([])
  const [filteredServices, setFilteredServices] = useState<SonographyService[]>([])
  const [loading, setLoading] = useState(true)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingService, setEditingService] = useState<SonographyService | null>(null)
  ...
  const fetchServices = async () => {
    try {
      ...
      console.error('Error fetching sonography services:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving sonography service:', error)
    }
  }

  const handleEdit = (service: SonographyService) => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this sonography service?')) {
      try {
        const { error } = await supabase
          .from('sonography_services')
          .delete()
        ...
        console.error('Error deleting sonography service:', error)
      }
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">Sonography Services</h1>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button>
                <Plus className="h-4 w-4 mr-2" />
                Add Sonography Service
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>
                  {editingService ? 'Edit Sonography Service' : 'Add New Sonography Service'}
                </DialogTitle>
              </DialogHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Sonography Services ({filteredServices.length})</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
